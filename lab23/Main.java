...
// Задание 1
final class Drink implements Itemm {
...
// Задание 2
final class Dish implements Itemm {
    private static final double DEFAULT_COST = 0.0;
    private static final String DEFAULT_NAME = "";
    private static final String DEFAULT_DESCRIPTION = "";
...
class InternetOrder {
    ...
    InternetOrder(Itemm[] items) {
        this();
        addItems(items);
    }

    private void addItems(Itemm[] items) {
        for (Itemm item : items) {
            addItem(item);
        }
    }

    public boolean addItem(Itemm item) {
        if (item == null) {
            return false;
        }
    ...
    public Itemm[] getSortedItemsByPrice() {
        Itemm[] items = getItems();
...
public class Main {
    public static void main(String[] args) {
        ...
        InternetOrder order = new InternetOrder(new Itemm[] {coffee, tea, pizza, salad});
        ...
        Itemm[] sortedItems = order.getSortedItemsByPrice();
        for (Itemm item : sortedItems) {
